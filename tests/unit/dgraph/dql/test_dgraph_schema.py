...
class DGraphSchemaTestCase(unittest.TestCase):
    ...
    def test_get_attributes_successful_no_duplicates(self) -> None:
        type_attribute = TypeAttribute(attr_name=TEST_ATTR_NAME,
                                       attr_type=TEST_ATTR_TYPE,
                                       is_list_type=TEST_IS_LIST_TYPE_TRUE,
    ...
    def test_get_attributes_successful_duplicates(self) -> None:
        type_attribute = TypeAttribute(attr_name=TEST_ATTR_NAME,
                                       attr_type=TEST_ATTR_TYPE,
                                       is_list_type=TEST_IS_LIST_TYPE_TRUE,
                                       indices=TEST_INDICES_1)
        ...
        dgraph_schema = DGraphSchema([node_schema, node_schema2])
        self.assertEqual(dgraph_schema.get_attributes(), [type_attribute])

    def test_get_attributes_error(self) -> None:
        type_attribute = TypeAttribute(attr_name=TEST_ATTR_NAME,
                                       attr_type=TEST_ATTR_TYPE,
                                       is_list_type=TEST_IS_LIST_TYPE_TRUE,
                                       indices=TEST_INDICES_1)
        ...
        def get_attributes() -> None:
            dgraph_schema.get_attributes()
        self.assertRaises(SchemaDuplicateError, get_attributes)
